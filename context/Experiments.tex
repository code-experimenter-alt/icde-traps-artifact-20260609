...
\begin{table}[t]
  \centering
  \footnotesize
  \caption{Backend verifier latency and parameters.}
  ...
  \begin{tabularx}{\columnwidth}{lXX}
    ...
    Parameters
      & \(t=3\), \(m=65536\) KiB, \(p=1\)
      & iter.\(=600000\), hash\(=\texttt{SHA-256}\), dkLen\(=32\) bytes \\
    Verifier calls
      & 78,620
      & 78,620 \\
    Mean
...
Ext. & Blocklist & Registration-time or policy-side weak-password denylist screening~\cite{nist_sp800_63b_4_2025,owasp_authentication_cheat_sheet}. \\
...
Diag. & Peppered tag & Truncated keyed fast-verifier tag with uniform or adaptive tag lengths. \\
...
Section~\ref{sec:blocks} discusses keyed tag fingerprints as a compatible fast-verification substrate. The frontier includes uniform peppered tags, adaptive account-risk tags, exact negative caches, and rate-limit-plus-tag hybrids. These rows define the tag-fingerprint operating point; the AMQ rows define the approximate-membership operating point where TRAPS's allocation, cap, replay, and migration policies are exercised.
...
\begin{figure}[t]
  ...
  \caption{Seeded admission-index frontier with operating baselines. AMQ rows use Bloom-style approximate membership; tag rows use truncated keyed fingerprints; rate+tag rows may throttle valid attempts.}
...
\paragraph{Seeded controlled-baseline frontier}
...
Figure~\ref{fig:icde-shift-grid} sweeps target-account skew and train/test risk drift at 12 and 16 bits/key; additional sensitivity runs vary replay fraction and occupancy injection. TRAPS improves over non-learned AMQs when high-risk misses remain aligned with validation-time regions, and the grid also identifies when a frozen allocation should be refreshed. Table~\ref{tab:shift-dynamic} gives the 16-bit worst-case accounting: the worst frozen cell reaches \(1.51\times10^{-3}\), while previous-window next-epoch allocation and drift guard bring the worst case back near Global Bloom. Large drift is therefore a measured rebuild/fallback condition in the control plane.
...
\input{context/generated/table_shift_dynamic_capacity}
\end{table}

\begin{figure*}[t]
  \centering
  \includegraphics[width=\linewidth]{figure/icde_experiments/icde_scale_update_sweep.pdf}
  \caption{Scale and update behavior at 16 bits per represented credential. Probe counts compare Global Bloom, TRAPS AMQs, and keyed tags under the same represented set.}
...
\begin{figure*}[t]
  ...
  \caption{LANL-conditioned routing/skew replay and router ablation. Controls shuffle account, time, or risk information to test whether routing uses enterprise metadata skew.}
...
\begin{table}[t]
  \centering
  \scriptsize
  \caption{Mechanism ablation at 16 bits/key over 30 seeds. Raw IFFR uses all invalid attempts, first-seen IFFR removes repeated invalid tuples, and cap violations count cells above the configured regional cap.}
  \label{tab:traps-ablation}
  \input{context/generated/table_traps_ablation}
\end{table}
...
\begin{figure}[t]
  ...
  \caption{Method-specific bounded-worker queueing with measured Argon2 service time and an 8\% valid-attempt mix.}
...
Figure~\ref{fig:icde-queueing} maps method-specific forwarding rates from the controlled frontier to backend-worker pressure using the 122.07 ms Argon2 mean service time and eight backend workers. The 8\% valid-attempt mix stresses legitimate-login service time while retaining a large invalid stream. All one-sided indexes keep utilization close because valid attempts dominate residual backend work; rate-limit hybrids are plotted with their measured valid-drop caveat rather than mixed into the one-sided frontier.
